...
vec3 materialSpecularColor0 = vec3(0.2, 0.3, 0.2);
float materialGlossinessPower = 100.0;
float materialGlossinessStrength = 5.0;

vec3 ambientColor(vec3 ambientLightColor, vec3 kA)
{
...
vec3 color(Camera camera, vec2 uv)
{
	...
	if(t>=0)
	{
		...
		vec3 ambient = ambientColor(ambientLightColor, kA);
