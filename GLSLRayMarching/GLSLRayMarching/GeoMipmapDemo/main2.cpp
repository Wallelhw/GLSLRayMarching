...
public:
	...
	{
		return numComponents * numSectionPerComponents * numQuadPerSections + IVector2(1, 1);
	}

	const Component& GetComponent(IVector2 i) const
	{
		return components[i];
	}

	Component& GetComponent(IVector2 i)
	{
		return components[i];
	}

	void GetQuads(const Frustum& f, std::vector<IVector2>& quads) const
	{
	}
